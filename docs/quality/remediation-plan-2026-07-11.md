...
| JOB-06 | done | Atomizować przejścia stanu i sekwencje eventów | cancel i worker nie wybierają tego samego numeru eventu; stan oraz event zapisują się razem | `fix(jobs): serialize state transitions and events` |
...
| REL-02 | done | Dodać prawdziwy package smoke | test uruchamia artefakt jako `stacklab` pod systemd, sprawdza readiness, frontend, login i upgrade A→B | `test(release): run package smoke under systemd` |
...
| GIT-01 | done | Nie usuwać obcego `index.lock` | lock bez udowodnionej własności daje `operation_in_progress`; commit używa bezpiecznego indexu tymczasowego lub transakcji | `fix(git): preserve external index locks` |
| DATA-02 | done | Wersjonować migracje SQLite | tabela wersji, transakcja per migracja, test upgrade z poprzedniego schematu i zgodność rollbacku | `refactor(store): add versioned migrations` |
...
| ID | Status | Krok | Kryterium odbioru | Planowany commit |
| --- | --- | --- | --- | --- |
| AUTH-05 | done | Uzgodnić sliding expiry cookie i DB | aktywna sesja nie wygasa według starego cookie; touch jest throttlowany; błędy DB zwracają 5xx zamiast 401 | `fix(auth): align sliding session expiry` |
...
| ID | Status | Krok | Kryterium odbioru | Planowany commit |
| --- | --- | --- | --- | --- |
| QA-01 | done | Podnieść pokrycie krytycznych pakietów | coverage artifact w CI; progi dla `httpapi`, `selfupdate`, `jobs`, `store`; testy `audit`, `retention`, `fsmeta` | seria `test(...): cover ...` |
| QA-02 | done | Rozszerzyć browser E2E | desktop i mobile Chromium; Settings, schedules, Docker Admin, Git, Files, terminal/logs/stats; preflight Docker i użyteczny trace | seria `test(e2e): cover ...` |
...
Weryfikacja UX-06 z 2026-07-12 objęła rozdzielenie niezależnych odczytów,
...
oraz poprawnym buildem produkcyjnym.
