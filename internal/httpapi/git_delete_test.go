...
func TestGitFileDeleteRequiresReviewAndRefreshesGitWithAudit(t *testing.T) {
	t.Parallel()
	_, served, cfg := newInternalTestHandler(t)
	cookies := loginInternalTestUser(t, served, "test-password")
	parent := filepath.Join(cfg.RootDir, "stacks", "demo")
	if err := os.MkdirAll(parent, 0o755); err != nil {
		t.Fatal(err)
	}
	target := filepath.Join(parent, "compose.yaml.bak-20260806T1108")
	...
	runInternalGit(t, cfg.RootDir, "commit", "--allow-empty", "-m", "Initial workspace")
	info, err := os.Stat(target)
	if err != nil {
		t.Fatal(err)
	}
	body := map[string]any{"path": "stacks/demo/compose.yaml.bak-20260806T1108", "expected_modified_at": info.ModTime()}
	for _, tc := range []struct {
		...
		{"missing timestamp", map[string]any{"path": "stacks/demo/compose.yaml.bak-20260806T1108"}, cookies, http.StatusBadRequest, "validation_failed"},
		{"stale", map[string]any{"path": "stacks/demo/compose.yaml.bak-20260806T1108", "expected_modified_at": info.ModTime().Add(-time.Second)}, cookies, http.StatusConflict, "edit_conflict"},
		{"reserved", map[string]any{"path": "stacks/demo/compose.yaml", "expected_modified_at": info.ModTime()}, cookies, http.StatusBadRequest, "reserved_path"},
		...
		t.Run(tc.name, func(t *testing.T) {
			response := performInternalJSONRequest(t, served, http.MethodDelete, "/api/git/workspace/file", tc.body, tc.cookies)
	...
	request := httptest.NewRequest(http.MethodDelete, "/api/git/workspace/file", strings.NewReader(string(encoded)))
	...
	response := performInternalJSONRequest(t, served, http.MethodDelete, "/api/git/workspace/file", body, cookies)
	if response.Code != http.StatusOK {
		t.Fatalf("deletion status=%d body=%s", response.Code, response.Body.String())
	}
	var deleted gitworkspace.DeleteFileResponse
	decodeInternalResponse(t, response, &deleted)
	if !deleted.Deleted || deleted.Path != "stacks/demo/compose.yaml.bak-20260806T1108" || deleted.AuditAction != "delete_config_file" {
	...
	if !status.Clean || len(status.Items) != 0 {
		t.Fatalf("untracked backup remains in Git Changes: %#v", status)
	}
