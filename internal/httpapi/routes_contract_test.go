...
func domainRouteContract() []domainRoute {
	return []domainRoute{
		...
		{http.MethodDelete, "/api/git/workspace/file", "DELETE /api/git/workspace/file"},
		...
		{http.MethodGet, "/api/stats/stacks", "GET /api/stats/stacks"},
...
func TestHandlerRegistersDomainRouteContract(t *testing.T) {
	...
	if len(routes) != 73 {
		t.Fatalf("route contract contains %d operations, want 73", len(routes))
	}
...
func TestDomainRoutesPreserveAuthenticationPolicy(t *testing.T) {
	...
	if protectedCount != 66 {
		t.Fatalf("protected route count = %d, want 66", protectedCount)
	}
}
